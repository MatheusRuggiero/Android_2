...
import br.com.tecnomotor.marvin.api.v1.interfaces.lotXid.sensor.RevisionSensorLotXidInterface
...
class RevisionSensorLotXidController(
        ...
        try {
            ...
            if (txtViewMessageProgress != null) {
                activity?.runOnUiThread {
                    txtViewMessageProgress!!.text = application.applicationContext.getString(R.string.revision_sensor)
            ...
            val call = BasicAuthClientDefault<RevisionSensorLotXidInterface>(
                mobileUser.userName!!,
                mobileUser.password!!,
                application.applicationContext
            )
                .create(RevisionSensorLotXidInterface::class.java)
                .getLotXid(params)

            call.enqueue(object : Callback<List<RevisionSensorSynchronize>> {
                override fun onFailure(
                    call: Call<List<RevisionSensorSynchronize>>,
                    t: Throwable
                ) {
                ...
                override fun onResponse(
                    call: Call<List<RevisionSensorSynchronize>>,
                    response: Response<List<RevisionSensorSynchronize>>
                ) {
                    if (response.isSuccessful) {
                        GlobalScope.launch(Dispatchers.IO) {
                            ...
                            if (!list.isNullOrEmpty()) {
                                revisionSensorRepository.saveListObjectSynchronized(list)
                            }
                            ...
                            if (!GlobalUtil.method.recoverSharedPreferences(
                                    spConfiguration,
                                    PARAMETER_REVISION_SENSOR_MAX_XID
                                ).isNullOrEmpty()
                            ...
                            if (xidSp > 0) {
                                ...
                                val xidDatabase = revisionSensorRepository.maxXid()
                            ...
                            if (!isSynchronizationBackground && list.isNullOrEmpty()) {
                                activity?.runOnUiThread {
                                    GlobalUtil.method.shortTimeMessageAlert(
                                        application.applicationContext,
                                        application.applicationContext.getString(R.string.not_values_revision_sensor)
                                    )
                                }
                                isCompletedSynchronization = true
                            }
    ...
    private fun nextLotXid() {
        ...
        val sensorPlatformLotXidController =
            SensorPlatformLotXidController(
                ...
                progressBarSynchronization,
                activity,
                sensorPlatformRepository.maxXid()
            )
