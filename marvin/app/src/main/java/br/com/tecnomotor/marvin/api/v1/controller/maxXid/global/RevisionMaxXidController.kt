...
import br.com.tecnomotor.marvin.api.v1.controller.maxXid.injector.TranslateTypePlanTestInjectorMaxXidController
import br.com.tecnomotor.marvin.api.v1.interfaces.maxXid.global.RevisionMaxXidInterface
...
class RevisionMaxXidController(
        ...
        try {
            ...
            if (txtViewMessageProgress != null) {
                activity?.runOnUiThread {
                    txtViewMessageProgress!!.text = application.applicationContext.getString(R.string.max_xid_revision)
                    btnStartSynchronization!!.isEnabled = false
                }
                isSynchronizationBackground = false
            }

            val call = BasicAuthClientDefault<RevisionMaxXidInterface>(
                mobileUser.userName!!,
                mobileUser.password!!,
                application.applicationContext
            )
                .create(RevisionMaxXidInterface::class.java)
                .getMax(params)
                    ...
                    if (response.isSuccessful) {
                        GlobalScope.launch(Dispatchers.IO) {
                            ...
                            if (xidSp == 0
                                && !GlobalUtil.method.recoverSharedPreferences(
                                    spConfiguration,
                                    PARAMETER_REVISION_MAX_XID_TRY_AGAIN
                                ).isNullOrEmpty()
                                && !GlobalUtil.method.recoverSharedPreferences(
                                    spConfiguration,
                                    PARAMETER_REVISION_MAX_XID_TRY_AGAIN
                                ...
                                if (xidSp == 0) {
                                    ...
                                    val revisionMaxXidController =
                                        RevisionMaxXidController(
                                            application,
    ...
    fun nextMaxXid() {
        ...
        val translateTypePlanTestInjectorMaxXidController = TranslateTypePlanTestInjectorMaxXidController(
            application,
            txtViewMessageProgress,
            btnStartSynchronization,
            progressBarSynchronization,
