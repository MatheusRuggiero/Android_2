...
class InjectorsReportsAdapter(
    private var list: List<InjectorReportEntity>,
    private var context: Context,
    private var listener : OnItemClickListener
) : RecyclerView.Adapter<InjectorsReportsAdapter.ViewHolder>() {
...
//        val view = LayoutInflater.from(parent.context).inflate(R.layout.recyclerview_bomb_points, parent, false)
    ...
    fun setData(list1: List<InjectorReportEntity>) {
        list = ArrayList()
        if(!list1.isNullOrEmpty()){
            list1.forEach {
                (list as ArrayList<InjectorReportEntity>).add(it)
            }
