...
import br.com.tecnomotor.marvin.api.v1.interfaces.lotXid.pump.PumpLotXidInterface
        ...
        try {
            ...
            if (txtViewMessageProgress != null) {
                activity?.runOnUiThread {
                    txtViewMessageProgress!!.text = application.applicationContext.getString(R.string.pump)
            ...
            val call = BasicAuthClientDefault<PumpLotXidInterface>(
                mobileUser.userName!!,
                mobileUser.password!!, application.applicationContext
            )
                .create(PumpLotXidInterface::class.java)
                .getLotXid(params)

            call.enqueue(object : Callback<List<PumpSynchronize>> {
                override fun onFailure(
                    call: Call<List<PumpSynchronize>>,
                    t: Throwable
                ) {
                ...
                override fun onResponse(
                    call: Call<List<PumpSynchronize>>,
                    response: Response<List<PumpSynchronize>>
                ) {
                    if (response.isSuccessful) {
                        GlobalScope.launch(Dispatchers.IO) {
                            ...
                            if (!list.isNullOrEmpty()) {
                                pumpRepository.saveListObjectSynchronized(list)
                            }

                            var xidSp = 0

                            if (!GlobalUtil.method.recoverSharedPreferences(spConfiguration, PARAMETER_PUMP_MAX_XID).isNullOrEmpty()
                                && !GlobalUtil.method.recoverSharedPreferences(spConfiguration, PARAMETER_PUMP_MAX_XID).isNullOrBlank()
                            ) {
                                xidSp = GlobalUtil.method.recoverSharedPreferences(spConfiguration, PARAMETER_PUMP_MAX_XID)!!.toInt()
                            }

                            if (xidSp > 0) {

                                val xidDatabase = pumpRepository.maxXid()

                                if (xidSp >= (xidDatabase + 1)) {
                                    //delay(20)
                                    val pumpLotXidController = PumpLotXidController(
                                        application,
                            ...
                            if (!isSynchronizationBackground && list.isNullOrEmpty()) {
                                activity?.runOnUiThread {
                                    GlobalUtil.method.shortTimeMessageAlert(
                                        application.applicationContext,
                                        application.applicationContext.getString(R.string.not_values_pump)
                                    )
                                }
                                isCompletedSynchronization = true
                            }
