...
class ValvulaTestsPlansAdapter(
    private var list: List<ValvePlanTestEntity>,
    private var context: Context,
    private var listener : OnItemClickListener
) : RecyclerView.Adapter<ValvulaTestsPlansAdapter.ViewHolder>() {
...
//        val view = LayoutInflater.from(parent.context).inflate(R.layout.recyclerview_bomb_points, parent, false)
    ...
    fun setData(list1: List<ValvePlanTestEntity>) {
        list = ArrayList()
        if(!list1.isNullOrEmpty()){
            list1.forEach {
                (list as ArrayList<ValvePlanTestEntity>).add(it)
            }
