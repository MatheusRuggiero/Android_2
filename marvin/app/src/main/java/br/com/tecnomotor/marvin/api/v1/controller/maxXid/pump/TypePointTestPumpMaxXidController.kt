...
import br.com.tecnomotor.marvin.api.v1.interfaces.maxXid.pump.TypePointTestPumpMaxXidInterface
...
class TypePointTestPumpMaxXidController(
        ...
        try {
            ...
            if (txtViewMessageProgress != null) {
                activity?.runOnUiThread {
                    txtViewMessageProgress!!.text = application.applicationContext.getString(R.string.max_xid_type_point_test_pump)
                    btnStartSynchronization!!.isEnabled = false
                }
                isSynchronizationBackground = false
            }
            val call = BasicAuthClientDefault<TypePointTestPumpMaxXidInterface>(
                mobileUser.userName!!,
                mobileUser.password!!,
                application.applicationContext
            )
                .create(TypePointTestPumpMaxXidInterface::class.java)
                .getMax(params)
                        ...
                        GlobalScope.launch(Dispatchers.IO) {
                            ...
                            if (valuesMaxXid != null
                                ...
                                GlobalUtil.method.saveSharedPreferences(
                                    spConfiguration,
                                    PARAMETER_TYPE_POINT_TEST_PUMP_MAX_XID,
                                    valuesMaxXid.xid
                                )
                                ...
                                if (!isSynchronizationBackground) {
                                    GlobalUtil.method.shortTimeMessageAlert(
                                        application.applicationContext,
                                        application.applicationContext.getString(R.string.not_values_type_pump_max_xid)
                                    )
                                }
                            }
                            ...
                            if (xidSp == 0
                                && !GlobalUtil.method.recoverSharedPreferences(
                                    spConfiguration,
                                    PARAMETER_TYPE_POINT_TEST_PUMP_MAX_XID_TRY_AGAIN
                                ).isNullOrEmpty()
                                && !GlobalUtil.method.recoverSharedPreferences(
                                    spConfiguration,
                                    PARAMETER_TYPE_POINT_TEST_PUMP_MAX_XID_TRY_AGAIN
    ...
    fun nextMaxXid() {
        ...
        val typePumpMaxXidController = TypePumpMaxXidController(
            application,
            txtViewMessageProgress,
            btnStartSynchronization,
            progressBarSynchronization,
