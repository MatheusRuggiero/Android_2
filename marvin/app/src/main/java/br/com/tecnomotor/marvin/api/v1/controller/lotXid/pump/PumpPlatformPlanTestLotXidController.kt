...
import br.com.tecnomotor.marvin.api.v1.interfaces.lotXid.pump.PumpPlatformPlanTestLotXidInterface
...
class PumpPlatformPlanTestLotXidController(
        ...
        try {
            ...
            if (txtViewMessageProgress != null) {
                activity?.runOnUiThread {
                    txtViewMessageProgress!!.text = application.applicationContext.getString(R.string.pump_platform_test_plan)
            ...
            val call = BasicAuthClientDefault<PumpPlatformPlanTestLotXidInterface>(
                mobileUser.userName!!,
                mobileUser.password!!,
                application.applicationContext
            )
                .create(PumpPlatformPlanTestLotXidInterface::class.java)
                .getLotXid(params)

            call.enqueue(object : Callback<List<PumpPlatformPlanSynchronize>> {
                override fun onFailure(
                    call: Call<List<PumpPlatformPlanSynchronize>>,
                    t: Throwable
                ) {
                ...
                override fun onResponse(
                    call: Call<List<PumpPlatformPlanSynchronize>>,
                    response: Response<List<PumpPlatformPlanSynchronize>>
                ) {
                    if (response.isSuccessful) {
                        GlobalScope.launch(Dispatchers.IO) {
                            ...
                            if (!list.isNullOrEmpty()) {
                                pumpPlatformPlanTestRepository.saveListObjectSynchronized(list)
                            }
                            ...
                            if (!GlobalUtil.method.recoverSharedPreferences(
                                    spConfiguration,
                                    PARAMETER_PUMP_PLATFORM_PLAN_TEST_MAX_XID
                                ).isNullOrEmpty()
                            ...
                            if (xidSp > 0) {
                                ...
                                val xidDatabase = pumpPlatformPlanTestRepository.maxXid()
                            ...
                            if (!isSynchronizationBackground && list.isNullOrEmpty()) {
                                activity?.runOnUiThread {
                                    GlobalUtil.method.shortTimeMessageAlert(
                                        application.applicationContext,
                                        application.applicationContext.getString(R.string.not_values_pump_platform_plan_test)
                                    )
                                }
                                isCompletedSynchronization = true
                            }
    ...
    private fun nextLotXid() {
        ...
        val translateTypePlanTestPumpLotXidController =
            TranslateTypePlanTestPumpLotXidController(
                ...
                progressBarSynchronization,
                activity,
                translateTypePlanTestPumpRepository.maxXid()
            )

        translateTypePlanTestPumpLotXidController.load()
    }
}