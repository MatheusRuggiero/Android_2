...
import br.com.tecnomotor.marvin.api.v1.controller.lotXid.global.TypePlanTestLotXidController
import br.com.tecnomotor.marvin.api.v1.interfaces.lotXid.injector.TypePointInjectorTestLotXidInterface
...
class TypePointInjectorTestLotXidController(
        ...
        try {
            ...
            if (txtViewMessageProgress != null) {
                activity?.runOnUiThread {
                    txtViewMessageProgress!!.text = application.applicationContext.getString(R.string.point_type_injector_test)
            ...
            val call = BasicAuthClientDefault<TypePointInjectorTestLotXidInterface>(
                mobileUser.userName!!,
                mobileUser.password!!,
                application.applicationContext
            )
                .create(TypePointInjectorTestLotXidInterface::class.java)
                .getLotXid(params)

            call.enqueue(object : Callback<List<TypePointInjectorTestSynchronize>> {
                override fun onFailure(
                    call: Call<List<TypePointInjectorTestSynchronize>>,
                    t: Throwable
                ) {
                ...
                override fun onResponse(
                    call: Call<List<TypePointInjectorTestSynchronize>>,
                    response: Response<List<TypePointInjectorTestSynchronize>>
                ) {
                    if (response.isSuccessful) {
                        GlobalScope.launch(Dispatchers.IO) {
                            val list = response.body()
                            var isCompletedSynchronization = false
                            val typeInjectorRepository = TypePointInjectorTestRepository(application)

                            if (!list.isNullOrEmpty()) {
                                typeInjectorRepository.saveListObjectSynchronized(list)
                            }
                            ...
                            if (xidSp > 0) {
                                ...
                                val xidDatabase = typeInjectorRepository.maxXid()

                                if (xidSp >= (xidDatabase + 1)) {
                                    //delay(20)
                                    val typePointInjectorTestLotXidController =
                                        TypePointInjectorTestLotXidController(
                                            application,
                                    ...
                                    typePointInjectorTestLotXidController.load()
                                } else {
                            ...
                            if (!isSynchronizationBackground && list.isNullOrEmpty()) {
                                activity?.runOnUiThread {
                                    GlobalUtil.method.shortTimeMessageAlert(
                                        application.applicationContext,
                                        application.applicationContext.getString(R.string.not_values_type_point_test_injector)
                                    )
                                }
                                isCompletedSynchronization = true
                            }
    ...
    private fun nextLotXid() {
        ...
        val typePlanTestLotXidController =
            TypePlanTestLotXidController(
                application,
                txtViewMessageProgress,
                btnStartSynchronization,
                progressBarSynchronization,
                activity,
                typePlanTestRepository.maxXid()
            )
