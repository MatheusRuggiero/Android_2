...
class SensorTestPlansAdapter(
    private var list: List<SensorTestPlanEntity>,
    private var context: Context,
    private var listener : OnItemClickListener
) : RecyclerView.Adapter<SensorTestPlansAdapter.ViewHolder>() {
...
//        val view = LayoutInflater.from(parent.context).inflate(R.layout.recyclerview_bomb_points, parent, false)
    ...
    fun setData(list1: List<SensorTestPlanEntity>) {
        list = ArrayList()
        if(!list1.isNullOrEmpty()){
            list1.forEach {
                (list as ArrayList<SensorTestPlanEntity>).add(it)
            }
