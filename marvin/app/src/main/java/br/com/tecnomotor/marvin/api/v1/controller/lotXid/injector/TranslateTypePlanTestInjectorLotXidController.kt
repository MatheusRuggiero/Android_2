...
import br.com.tecnomotor.marvin.api.v1.interfaces.lotXid.injector.TranslateTypePlanTestInjectorLotXidInterface
import br.com.tecnomotor.marvin.api.v1.model.injector.TranslateTypePlanTestInjectorSynchronize
import br.com.tecnomotor.marvin.repository.injector.TranslateTypePlanTestInjectorRepository
import br.com.tecnomotor.marvin.utils.constant.ParameterSharedPreferences.PARAMETER_TRANSLATE_TYPE_PLAN_TEST_INJECTOR_MAX_XID
...
class TranslateTypePlanTestInjectorLotXidController(
        ...
        try {
            ...
            if (txtViewMessageProgress != null) {
                activity?.runOnUiThread {
                    txtViewMessageProgress!!.text = application.applicationContext.getString(R.string.translate_type_plan_test_injector)
            ...
            val call = BasicAuthClientDefault<TranslateTypePlanTestInjectorLotXidInterface>(
                mobileUser.userName!!,
                mobileUser.password!!,
                application.applicationContext
            )
                .create(TranslateTypePlanTestInjectorLotXidInterface::class.java)
                .getLotXid(params)

            call.enqueue(object : Callback<List<TranslateTypePlanTestInjectorSynchronize>> {
                override fun onFailure(
                    call: Call<List<TranslateTypePlanTestInjectorSynchronize>>,
                    t: Throwable
                ) {
                ...
                override fun onResponse(
                    call: Call<List<TranslateTypePlanTestInjectorSynchronize>>,
                    response: Response<List<TranslateTypePlanTestInjectorSynchronize>>
                ) {
                    if (response.isSuccessful) {
                        GlobalScope.launch(Dispatchers.IO) {
                            val list = response.body()
                            var isCompletedSynchronization = false
                            val translateTypePlanTestInjectorRepository = TranslateTypePlanTestInjectorRepository(application)
                            if (!list.isNullOrEmpty()) {
                                translateTypePlanTestInjectorRepository.saveListObjectSynchronized(list)
                            }
                            ...
                            if (!GlobalUtil.method.recoverSharedPreferences(
                                    spConfiguration,
                                    PARAMETER_TRANSLATE_TYPE_PLAN_TEST_INJECTOR_MAX_XID
                                ).isNullOrEmpty()
                            ...
                            if (xidSp > 0) {
                                ...
                                val xidDatabase = translateTypePlanTestInjectorRepository.maxXid()

                                if (xidSp >= (xidDatabase + 1)) {
                                    //delay(20)
                                    val translateTypePlanTestInjectorLotXidController =
                                        TranslateTypePlanTestInjectorLotXidController(
                                            application,
                                    ...
                                    translateTypePlanTestInjectorLotXidController.load()
                                } else {
                            ...
                            if (!isSynchronizationBackground && list.isNullOrEmpty()) {
                                activity?.runOnUiThread {
                                    GlobalUtil.method.shortTimeMessageAlert(
                                        application.applicationContext,
                                        application.applicationContext.getString(R.string.not_values_translate_type_plan_test_injector)
                                    )
                                }
                                isCompletedSynchronization = true
                            }
    ...
    private fun nextLotXid() {
        val translateTypePointInjectorRepository = TranslateTypePlanTestInjectorRepository(application)
        val translateTypePointTestInjectorLotXidController =
            TranslateTypePointTestInjectorLotXidController(
                ...
                progressBarSynchronization,
                activity,
                translateTypePointInjectorRepository.maxXid()
            )
