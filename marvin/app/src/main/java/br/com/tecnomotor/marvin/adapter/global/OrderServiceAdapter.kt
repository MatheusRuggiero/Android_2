...
class OrderServiceAdapter(
    private var list: List<OrderServiceEntity>,
    private var context: Context,
    private var listener : OnItemClickListener
) : RecyclerView.Adapter<OrderServiceAdapter.ViewHolder>() {
...
//        val view = LayoutInflater.from(parent.context).inflate(R.layout.recyclerview_bomb_points, parent, false)
    ...
    fun setData(list1: List<OrderServiceEntity>) {
        list = ArrayList()
        if(!list1.isNullOrEmpty()){
            list1.forEach {
                (list as ArrayList<OrderServiceEntity>).add(it)
            }
