...
class PumpsPlatformAdapter(
    private var list: List<PumpPlatformEntity>,
    private var context: Context,
    private var listener : OnItemClickListener
) : RecyclerView.Adapter<PumpsPlatformAdapter.ViewHolder>() {
...
//        val view = LayoutInflater.from(parent.context).inflate(R.layout.recyclerview_bomb_points, parent, false)
    ...
    fun setData(list1: List<PumpPlatformEntity>) {
        list = ArrayList()
        if(!list1.isNullOrEmpty()){
            list1.forEach {
                (list as ArrayList<PumpPlatformEntity>).add(it)
            }
