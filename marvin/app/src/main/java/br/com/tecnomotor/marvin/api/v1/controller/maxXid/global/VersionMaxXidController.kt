...
import br.com.tecnomotor.marvin.api.v1.controller.maxXid.sensor.RevisionSensorMaxXidController
import br.com.tecnomotor.marvin.api.v1.interfaces.maxXid.global.VersionMaxXidInterface
...
class VersionMaxXidController(
        ...
        try {
            ...
            if (txtViewMessageProgress != null) {
                activity?.runOnUiThread {
                    txtViewMessageProgress!!.text = application.applicationContext.getString(R.string.max_xid_version)
                    btnStartSynchronization!!.isEnabled = false
                }
                isSynchronizationBackground = false
            }

            val call = BasicAuthClientDefault<VersionMaxXidInterface>(
                mobileUser.userName!!,
                mobileUser.password!!,
                application.applicationContext
            )
                .create(VersionMaxXidInterface::class.java)
                .getMax(params)
                    ...
                    if (response.isSuccessful) {
                        GlobalScope.launch(Dispatchers.IO) {
                            ...
                            if (xidSp == 0
                                && !GlobalUtil.method.recoverSharedPreferences(
                                    spConfiguration,
                                    PARAMETER_VERSION_MAX_XID_TRY_AGAIN
                                ).isNullOrEmpty()
                                && !GlobalUtil.method.recoverSharedPreferences(
                                    spConfiguration,
                                    PARAMETER_VERSION_MAX_XID_TRY_AGAIN
                                ...
                                if (xidSp == 0) {
                                    ...
                                    val versionMaxXidController =
                                        VersionMaxXidController(
                                            application,
    ...
    fun nextMaxXid() {
        val revisionSensorMaxXidController = RevisionSensorMaxXidController(
            application,
            txtViewMessageProgress,
            btnStartSynchronization,
            progressBarSynchronization,
