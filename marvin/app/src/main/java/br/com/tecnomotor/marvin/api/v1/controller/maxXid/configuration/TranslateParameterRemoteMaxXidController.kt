package br.com.tecnomotor.marvin.api.v1.controller.maxXid.configuration
...
import br.com.tecnomotor.marvin.api.v1.controller.lotXid.injector.TypePointInjectorTestLotXidController
import br.com.tecnomotor.marvin.api.v1.interfaces.maxXid.global.TypePlanTestMaxXidInterface
...
class TranslateParameterRemoteMaxXidController(
        ...
        try {
            ...
            if (txtViewMessageProgress != null) {
                activity?.runOnUiThread {
                    txtViewMessageProgress!!.text = application.applicationContext.getString(R.string.max_xid_translate_parameter_remote)
                    btnStartSynchronization!!.isEnabled = false
                }
                isSynchronizationBackground = false
            }

            val call = BasicAuthClientDefault<TypePlanTestMaxXidInterface>(
                mobileUser.userName!!,
                mobileUser.password!!,
                application.applicationContext
            )
                .create(TypePlanTestMaxXidInterface::class.java)
                .getMax(params)
                        ...
                        GlobalScope.launch(Dispatchers.IO) {
                            ...
                            if (valuesMaxXid != null
                                ...
                                if (!isSynchronizationBackground) {
                                    GlobalUtil.method.shortTimeMessageAlert(
                                        application.applicationContext,
                                        application.applicationContext.getString(R.string.not_values_translate_parameter_remote_max_xid)
                                    )
                                }
                            }
                            ...
                            if (xidSp == 0
                                ...
                                if (xidSp == 0) {
                                    ...
                                    val translateParameterRemoteMaxXidController =
                                        TranslateParameterRemoteMaxXidController(
                                            application,
                                    ...
                                    translateParameterRemoteMaxXidController.load()
    ...
    fun nextMaxXid() {
        ...
        val typePointInjectorRepository = TypePointInjectorTestRepository(application)

        val typePointInjectorTestLotXidController = TypePointInjectorTestLotXidController(
            application,
            txtViewMessageProgress,
            btnStartSynchronization,
            progressBarSynchronization,
            activity,
            typePointInjectorRepository.maxXid()
        )
