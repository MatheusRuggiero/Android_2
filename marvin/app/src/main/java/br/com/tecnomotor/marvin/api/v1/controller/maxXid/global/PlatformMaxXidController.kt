...
import br.com.tecnomotor.marvin.api.v1.controller.maxXid.injector.PointTestInjectorMaxXidController
import br.com.tecnomotor.marvin.api.v1.interfaces.maxXid.global.PlatformMaxXidInterface
...
class PlatformMaxXidController(
        ...
        try {
            ...
            if (txtViewMessageProgress != null) {
                activity?.runOnUiThread {
                    txtViewMessageProgress!!.text = application.applicationContext.getString(R.string.max_xid_platform)
                    btnStartSynchronization!!.isEnabled = false
                }
                isSynchronizationBackground = false
            }

            val call = BasicAuthClientDefault<PlatformMaxXidInterface>(
                mobileUser.userName!!,
                mobileUser.password!!,
                application.applicationContext
            )
                .create(PlatformMaxXidInterface::class.java)
                .getMax(params)
                    ...
                    if (response.isSuccessful) {
                        GlobalScope.launch(Dispatchers.IO) {
                            ...
                            if (xidSp == 0
                                && !GlobalUtil.method.recoverSharedPreferences(
                                    spConfiguration,
                                    PARAMETER_PLATFORM_MAX_XID_TRY_AGAIN
                                ).isNullOrEmpty()
                                && !GlobalUtil.method.recoverSharedPreferences(
                                    spConfiguration,
                                    PARAMETER_PLATFORM_MAX_XID_TRY_AGAIN
                                ...
                                if (xidSp == 0) {
                                    ...
                                    val platformMaxXidController =
                                        PlatformMaxXidController(
                                            application,
    ...
    fun nextMaxXid() {
        ...
        val pointInjectorTestMaxXidController = PointTestInjectorMaxXidController(
            application,
            txtViewMessageProgress,
            btnStartSynchronization,
            progressBarSynchronization,
