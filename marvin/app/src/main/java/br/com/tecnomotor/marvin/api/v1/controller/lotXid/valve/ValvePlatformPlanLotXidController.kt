...
import br.com.tecnomotor.marvin.api.v1.interfaces.lotXid.valve.ValvePlatformPlanLotXidInterface
...
import br.com.tecnomotor.marvin.repository.valve.ValvePlatformPlanRepository
...
class ValvePlatformPlanLotXidController(
        ...
        try {
            ...
            if (txtViewMessageProgress != null) {
                activity?.runOnUiThread {
                    txtViewMessageProgress!!.text = application.applicationContext.getString(R.string.valve_platform_plan)
            ...
            val call = BasicAuthClientDefault<ValvePlatformPlanLotXidInterface>(
                mobileUser.userName!!,
                mobileUser.password!!,
                application.applicationContext
            )
                .create(ValvePlatformPlanLotXidInterface::class.java)
                .getLotXid(params)

            call.enqueue(object : Callback<List<ValvePlatformPlanSynchronize>> {
                override fun onFailure(
                    call: Call<List<ValvePlatformPlanSynchronize>>,
                    t: Throwable
                ) {
                ...
                override fun onResponse(
                    call: Call<List<ValvePlatformPlanSynchronize>>,
                    response: Response<List<ValvePlatformPlanSynchronize>>
                ) {
                    if (response.isSuccessful) {
                        GlobalScope.launch(Dispatchers.IO) {
                            val list = response.body()
                            var isCompletedSynchronization = false
                            val valvePlatformPlanRepository = ValvePlatformPlanRepository(application)
                            if (!list.isNullOrEmpty()) {
                                valvePlatformPlanRepository.saveListObjectSynchronized(list)
                            }
                            ...
                            if (!GlobalUtil.method.recoverSharedPreferences(
                                    spConfiguration,
                                    PARAMETER_VALVE_PLATFORM_PLAN_MAX_XID
                                ).isNullOrEmpty()
                            ...
                            if (xidSp > 0) {
                                ...
                                val xidDatabase = valvePlatformPlanRepository.maxXid()

                                if (xidSp >= (xidDatabase + 1)) {
                                    //delay(20)
                                    val valvePlatformPlanLotXidController =
                                        ValvePlatformPlanLotXidController(
                                            application,
                                    ...
                                    valvePlatformPlanLotXidController.load()
                                } else {
                            ...
                            if (!isSynchronizationBackground && list.isNullOrEmpty()) {
                                activity?.runOnUiThread {
                                    GlobalUtil.method.shortTimeMessageAlert(
                                        application.applicationContext,
                                        application.applicationContext.getString(R.string.not_values_valve_platform_plan)
                                    )
                                }
                                isCompletedSynchronization = true
                            }
    ...
    private fun nextLotXid() {
        ...
        val translateTypePlanTestValveLotXidController =
            TranslateTypePlanTestValveLotXidController(
                ...
                progressBarSynchronization,
                activity,
                translateTypePlanTestValveRepository.maxXid()
            )
