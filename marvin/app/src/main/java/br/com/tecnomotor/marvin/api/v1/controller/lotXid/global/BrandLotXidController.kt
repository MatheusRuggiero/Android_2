...
import br.com.tecnomotor.marvin.api.v1.controller.lotXid.injector.InjectorLotXidController
import br.com.tecnomotor.marvin.api.v1.interfaces.lotXid.global.BrandLotXidInterface
...
class BrandLotXidController(
        ...
        try {
            ...
            if (txtViewMessageProgress != null) {
                activity?.runOnUiThread {
                    txtViewMessageProgress!!.text = application.applicationContext.getString(R.string.brand)
            ...
            val call = BasicAuthClientDefault<BrandLotXidInterface>(
                mobileUser.userName!!,
                mobileUser.password!!,
                application.applicationContext
            )
                .create(BrandLotXidInterface::class.java)
                .getLotXid(params)

            call.enqueue(object : Callback<List<BrandSynchronize>> {
                override fun onFailure(call: Call<List<BrandSynchronize>>, t: Throwable) {
                ...
                override fun onResponse(
                    call: Call<List<BrandSynchronize>>,
                    response: Response<List<BrandSynchronize>>) {
                    if (response.isSuccessful) {
                        GlobalScope.launch(Dispatchers.IO) {
                            ...
                            if (!list.isNullOrEmpty()) {
                                brandRepository.saveListObjectSynchronized(list)
                            }
                            ...
                            if (xidSp > 0) {
                                ...
                                val xidDatabase = brandRepository.maxXid()
                                if (xidSp >= (xidDatabase + 1)) {
                                    //delay(20)
                                    val brandLotXidController =
                                        BrandLotXidController(
                                            application,
                            ...
                            if (!isSynchronizationBackground && list.isNullOrEmpty()) {
                                activity?.runOnUiThread {
                                    GlobalUtil.method.shortTimeMessageAlert(
                                        application.applicationContext,
                                        application.applicationContext.getString(R.string.not_values_brand)
                                    )
                                }
                                isCompletedSynchronization = true
                            }
    ...
    private fun nextLotXid() {
        ...
        val injectorLotXidController =
            InjectorLotXidController(
                application,
                txtViewMessageProgress,
                btnStartSynchronization,
                progressBarSynchronization,
                activity,
                injectorRepository.maxXid()
            )
