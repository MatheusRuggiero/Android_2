...
import br.com.tecnomotor.marvin.api.v1.interfaces.lotXid.injector.RevisionInjectorLotXidInterface
...
class RevisionInjectorLotXidController(
        ...
        try {
            ...
            if (txtViewMessageProgress != null) {
                activity?.runOnUiThread {
                    txtViewMessageProgress!!.text = application.applicationContext.getString(R.string.revision_injector)
            ...
            val call = BasicAuthClientDefault<RevisionInjectorLotXidInterface>(
                mobileUser.userName!!,
                mobileUser.password!!,
                application.applicationContext
            )
                .create(RevisionInjectorLotXidInterface::class.java)
                .getLotXid(params)

            call.enqueue(object : Callback<List<RevisionInjectorSynchronize>> {
                override fun onFailure(
                    call: Call<List<RevisionInjectorSynchronize>>,
                    t: Throwable
                ) {
                ...
                override fun onResponse(
                    call: Call<List<RevisionInjectorSynchronize>>,
                    response: Response<List<RevisionInjectorSynchronize>>
                ) {
                    if (response.isSuccessful) {
                        GlobalScope.launch(Dispatchers.IO) {
                            ...
                            if (!list.isNullOrEmpty()) {
                                revisionInjectorRepository.saveListObjectSynchronized(list)
                            }
                            ...
                            if (xidSp > 0) {
                                ...
                                val xidDatabase = revisionInjectorRepository.maxXid()
                            ...
                            if (!isSynchronizationBackground && list.isNullOrEmpty()) {
                                activity?.runOnUiThread {
                                    GlobalUtil.method.shortTimeMessageAlert(
                                        application.applicationContext,
                                        application.applicationContext.getString(R.string.not_values_revision_injector)
                                    )
                                }
                                isCompletedSynchronization = true
                            }
    ...
    private fun nextLotXid() {
        ...
        val injectorPlatformLotXidController =
            InjectorPlatformLotXidController(
                ...
                progressBarSynchronization,
                activity,
                injectorPlatformRepository.maxXid()
            )
