...
import br.com.tecnomotor.marvin.api.v1.interfaces.lotXid.injector.PointTestInjectorLotXidInterface
...
class PointTestInjectorLotXidController(
        ...
        try {
            ...
            if (txtViewMessageProgress != null) {
                activity?.runOnUiThread {
                    txtViewMessageProgress!!.text = application.applicationContext.getString(R.string.point_test_injector)
            ...
            val call = BasicAuthClientDefault<PointTestInjectorLotXidInterface>(
                mobileUser.userName!!,
                mobileUser.password!!,
                application.applicationContext
            )
                .create(PointTestInjectorLotXidInterface::class.java)
                .getLotXid(params)

            call.enqueue(object : Callback<List<PointTestInjectorSynchronize>> {
                override fun onFailure(
                    call: Call<List<PointTestInjectorSynchronize>>,
                    t: Throwable
                ) {
                ...
                override fun onResponse(
                    call: Call<List<PointTestInjectorSynchronize>>,
                    response: Response<List<PointTestInjectorSynchronize>>
                ) {
                    if (response.isSuccessful) {
                        GlobalScope.launch(Dispatchers.IO) {
                            ...
                            if (!list.isNullOrEmpty()) {
                                pointTestInjectorRepository.saveListObjectSynchronized(list)
                            }
                            ...
                            if (!GlobalUtil.method.recoverSharedPreferences(
                                    spConfiguration,
                                    PARAMETER_POINT_TEST_INJECTOR_MAX_XID
                                ).isNullOrEmpty()
                            ...
                            if (xidSp > 0) {
                                ...
                                val xidDatabase = pointTestInjectorRepository.maxXid()
                            ...
                            if (!isSynchronizationBackground && list.isNullOrEmpty()) {
                                activity?.runOnUiThread {
                                    GlobalUtil.method.shortTimeMessageAlert(
                                        application.applicationContext,
                                        application.applicationContext.getString(R.string.not_values_point_injector_test)
                                    )
                                }
                                isCompletedSynchronization = true
                            }
                            ...
                            if (isCompletedSynchronization) {
                                if (progressBarSynchronization != null) {
                                    progressBarSynchronization!!.progress =
                                        (progressBarSynchronization!!.progress + ValueDefault.VALUES_PROGRESS_BAR)
                                }
                                nextLotXid()
                            }
                        }
