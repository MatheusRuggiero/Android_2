...
import br.com.tecnomotor.marvin.api.v1.controller.maxXid.global.RevisionMaxXidController
import br.com.tecnomotor.marvin.api.v1.interfaces.maxXid.injector.RevisionInjectorMaxXidInterface
...
class RevisionInjectorMaxXidController(
        ...
        try {
            ...
            if (txtViewMessageProgress != null) {
                activity?.runOnUiThread {
                    txtViewMessageProgress!!.text = application.applicationContext.getString(R.string.max_xid_revision_injector)
                    btnStartSynchronization!!.isEnabled = false
                }
                isSynchronizationBackground = false
            }

            val call = BasicAuthClientDefault<RevisionInjectorMaxXidInterface>(
                mobileUser.userName!!,
                mobileUser.password!!,
                application.applicationContext
            )
                .create(RevisionInjectorMaxXidInterface::class.java)
                .getMax(params)
                    ...
                    if (response.isSuccessful) {
                        GlobalScope.launch(Dispatchers.IO) {
                            ...
                            if (valuesMaxXid != null
                                ...
                                PARAMETER_REVISION_INJECTOR_MAX_XID
                                ...
                                if (!isSynchronizationBackground) {
                                    GlobalUtil.method.shortTimeMessageAlert(
                                        application.applicationContext,
                                        application.applicationContext.getString(R.string.not_values_revision_injector_max_xid)
                                    )
                                }
                            }
                            ...
                            if (xidSp == 0
                                && !GlobalUtil.method.recoverSharedPreferences(
                                    spConfiguration,
                                    PARAMETER_REVISION_INJECTOR_MAX_XID_TRY_AGAIN
                                ).isNullOrEmpty()
                                && !GlobalUtil.method.recoverSharedPreferences(
                                    spConfiguration,
                                    PARAMETER_REVISION_INJECTOR_MAX_XID_TRY_AGAIN
                                ...
                                if (xidSp == 0) {
                                    ...
                                    val revisionInjectorMaxXidController =
                                        RevisionInjectorMaxXidController(
                                            application,
                                    ...
                                    revisionInjectorMaxXidController.load()
    ...
    fun nextMaxXid() {
        ...
        val revisionMaxXidController = RevisionMaxXidController(
            application,
            txtViewMessageProgress,
            btnStartSynchronization,
            progressBarSynchronization,
