...
import br.com.tecnomotor.marvin.api.v1.interfaces.maxXid.pump.PumpPlatformPlanTestMaxXidInterface
...
class PumpPlatformPlanTestMaxXidController(
        ...
        try {
            ...
            if (txtViewMessageProgress != null) {
                activity?.runOnUiThread {
                    txtViewMessageProgress!!.text = application.applicationContext.getString(R.string.max_xid_pump_platform_plan_test)
                    btnStartSynchronization!!.isEnabled = false
                }
                isSynchronizationBackground = false
            }
            val call = BasicAuthClientDefault<PumpPlatformPlanTestMaxXidInterface>(
                mobileUser.userName!!,
                mobileUser.password!!,
                application.applicationContext
            )
                .create(PumpPlatformPlanTestMaxXidInterface::class.java)
                .getMax(params)
                        ...
                        GlobalScope.launch(Dispatchers.IO) {
                            ...
                            if (valuesMaxXid != null
                                ...
                                if (!isSynchronizationBackground) {
                                    GlobalUtil.method.shortTimeMessageAlert(
                                        application.applicationContext,
                                        application.applicationContext.getString(R.string.not_values_pump_platform_plan_test_max_xid)
                                    )
                                }
                            }
                            ...
                            if (xidSp == 0
                                && !GlobalUtil.method.recoverSharedPreferences(
                                    spConfiguration,
                                    PARAMETER_PUMP_PLATFORM_PLAN_TEST_MAX_XID_TRY_AGAIN
                                ).isNullOrEmpty()
                                && !GlobalUtil.method.recoverSharedPreferences(
                                    spConfiguration,
                                    PARAMETER_PUMP_PLATFORM_PLAN_TEST_MAX_XID_TRY_AGAIN
                                ...
                                if (xidSp == 0) {
                                    ...
                                    val pumpPlatformPlanTestMaxXidController = PumpPlatformPlanTestMaxXidController(
                                        application,
                                    ...
                                    pumpPlatformPlanTestMaxXidController.load()
    ...
    fun nextMaxXid() {
        val revisionPumpMaxXidController = RevisionPumpMaxXidController(
            application,
            txtViewMessageProgress,
            btnStartSynchronization,
            progressBarSynchronization,
        ...
        revisionPumpMaxXidController.load()
    }
}