...
import br.com.tecnomotor.marvin.api.v1.interfaces.maxXid.valve.ValvePlanTestMaxXidInterface
...
class ValvePlanTestMaxXidController(
        ...
        try {
            ...
            if (txtViewMessageProgress != null) {
                activity?.runOnUiThread {
                    txtViewMessageProgress!!.text = application.applicationContext.getString(R.string.max_xid_plan_test_valve)
                    btnStartSynchronization!!.isEnabled = false
                }
                isSynchronizationBackground = false
            }
            val call = BasicAuthClientDefault<ValvePlanTestMaxXidInterface>(
                mobileUser.userName!!,
                mobileUser.password!!,
                application.applicationContext
            )
                .create(ValvePlanTestMaxXidInterface::class.java)
                .getMax(params)
                        ...
                        GlobalScope.launch(Dispatchers.IO) {
                            ...
                            if (valuesMaxXid != null
                                ...
                                if (!isSynchronizationBackground) {
                                    GlobalUtil.method.shortTimeMessageAlert(
                                        application.applicationContext,
                                        application.applicationContext.getString(R.string.not_values_plan_test_valve_max_xid)
                                    )
                                }
                            }
                            ...
                            if (xidSp == 0
                                && !GlobalUtil.method.recoverSharedPreferences(
                                    spConfiguration,
                                    PARAMETER_VALVE_PLAN_TEST_MAX_XID_TRY_AGAIN
                                ).isNullOrEmpty()
                                && !GlobalUtil.method.recoverSharedPreferences(
                                    spConfiguration,
                                    PARAMETER_VALVE_PLAN_TEST_MAX_XID_TRY_AGAIN
                                ...
                                if (xidSp == 0) {
                                    ...
                                    val valveTestMaxXidController = ValvePlanTestMaxXidController(
                                        application,
                                    ...
                                    valveTestMaxXidController.load()
    ...
    fun nextMaxXid() {
        val valvePlatformMaxXidController = ValvePlatformMaxXidController(
            application,
            txtViewMessageProgress,
            btnStartSynchronization,
            progressBarSynchronization,
