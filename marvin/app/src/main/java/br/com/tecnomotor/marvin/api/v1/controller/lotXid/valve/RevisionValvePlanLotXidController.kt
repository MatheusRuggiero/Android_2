...
import br.com.tecnomotor.marvin.api.v1.interfaces.lotXid.valve.RevisionValvePlanLotXidInterface
...
class RevisionValvePlanLotXidController(
        ...
        try {
            ...
            if (txtViewMessageProgress != null) {
                activity?.runOnUiThread {
                    txtViewMessageProgress!!.text = application.applicationContext.getString(R.string.revision_valve_plan)
            ...
            val call = BasicAuthClientDefault<RevisionValvePlanLotXidInterface>(
                mobileUser.userName!!,
                mobileUser.password!!,
                application.applicationContext
            )
                .create(RevisionValvePlanLotXidInterface::class.java)
                .getLotXid(params)

            call.enqueue(object : Callback<List<RevisionPlanValveSynchronize>> {
                override fun onFailure(
                    call: Call<List<RevisionPlanValveSynchronize>>,
                    t: Throwable
                ) {
                ...
                override fun onResponse(
                    call: Call<List<RevisionPlanValveSynchronize>>,
                    response: Response<List<RevisionPlanValveSynchronize>>
                ) {
                    if (response.isSuccessful) {
                        GlobalScope.launch(Dispatchers.IO) {
                            ...
                            if (!list.isNullOrEmpty()) {
                                revisionValvePlanRepository.saveListObjectSynchronized(list)
                            }
                            ...
                            if (!GlobalUtil.method.recoverSharedPreferences(
                                    spConfiguration,
                                    PARAMETER_REVISION_VALVE_PLAN_MAX_XID
                                ).isNullOrEmpty()
                            ...
                            if (xidSp > 0) {
                                ...
                                val xidDatabase = revisionValvePlanRepository.maxXid()
                            ...
                            if (!isSynchronizationBackground && list.isNullOrEmpty()) {
                                activity?.runOnUiThread {
                                    GlobalUtil.method.shortTimeMessageAlert(
                                        application.applicationContext,
                                        application.applicationContext.getString(R.string.not_values_revision_valve_plan)
                                    )
                                }
                                isCompletedSynchronization = true
                            }
    ...
    private fun nextLotXid() {
        ...
        val valvePlatformPlanLotXidController =
            ValvePlatformPlanLotXidController(
                ...
                progressBarSynchronization,
                activity,
                valvePlatformPlanRepository.maxXid()
            )
