...
import br.com.tecnomotor.marvin.api.v1.interfaces.lotXid.pump.TranslateTypePlanTestPumpLotXidInterface
import br.com.tecnomotor.marvin.api.v1.model.pump.TranslateTypePlanTestPumpSynchronize
import br.com.tecnomotor.marvin.repository.pump.TranslateTypePlanTestPumpRepository
...
class TranslateTypePlanTestPumpLotXidController(
        ...
        try {
            ...
            if (txtViewMessageProgress != null) {
                activity?.runOnUiThread {
                    txtViewMessageProgress!!.text = application.applicationContext.getString(R.string.translate_type_plan_test_pump)
            ...
            val call = BasicAuthClientDefault<TranslateTypePlanTestPumpLotXidInterface>(
                mobileUser.userName!!,
                mobileUser.password!!,
                application.applicationContext
            )
                .create(TranslateTypePlanTestPumpLotXidInterface::class.java)
                .getLotXid(params)

            call.enqueue(object : Callback<List<TranslateTypePlanTestPumpSynchronize>> {
                override fun onFailure(
                    call: Call<List<TranslateTypePlanTestPumpSynchronize>>,
                    t: Throwable
                ) {
                ...
                override fun onResponse(
                    call: Call<List<TranslateTypePlanTestPumpSynchronize>>,
                    response: Response<List<TranslateTypePlanTestPumpSynchronize>>
                ) {
                    if (response.isSuccessful) {
                        GlobalScope.launch(Dispatchers.IO) {
                            val list = response.body()
                            var isCompletedSynchronization = false
                            val translateTypePlanTestPumpRepository = TranslateTypePlanTestPumpRepository(application)
                            if (!list.isNullOrEmpty()) {
                                translateTypePlanTestPumpRepository.saveListObjectSynchronized(list)
                            }
                            ...
                            if (!GlobalUtil.method.recoverSharedPreferences(
                                    spConfiguration,
                                    PARAMETER_TRANSLATE_TYPE_PLAN_PUMP_MAX_XID
                                ).isNullOrEmpty()
                            ...
                            if (xidSp > 0) {
                                ...
                                val xidDatabase = translateTypePlanTestPumpRepository.maxXid()

                                if (xidSp >= (xidDatabase + 1)) {
                                    //delay(20)
                                    val translateTypePlanTestInjectorLotXidController =
                                        TranslateTypePlanTestPumpLotXidController(
                                            application,
                            ...
                            if (!isSynchronizationBackground && list.isNullOrEmpty()) {
                                activity?.runOnUiThread {
                                    GlobalUtil.method.shortTimeMessageAlert(
                                        application.applicationContext,
                                        application.applicationContext.getString(R.string.not_values_translate_type_plan_test_pump)
                                    )
                                }
                                isCompletedSynchronization = true
                            }
    ...
    private fun nextLotXid() {
        ...
        val translateTypePointTestPumpLotXidController =
            TranslateTypePointTestPumpLotXidController(
                ...
                progressBarSynchronization,
                activity,
                translateTypePointTestPumpRepository.maxXid()
            )
        translateTypePointTestPumpLotXidController.load()
    }
}