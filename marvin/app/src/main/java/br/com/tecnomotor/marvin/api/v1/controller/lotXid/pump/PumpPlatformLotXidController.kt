...
import br.com.tecnomotor.marvin.api.v1.interfaces.lotXid.pump.PumpPlatformLotXidInterface
...
class PumpPlatformLotXidController(
        ...
        try {
            ...
            if (txtViewMessageProgress != null) {
                activity?.runOnUiThread {
                    txtViewMessageProgress!!.text = application.applicationContext.getString(R.string.pump_platform)
            ...
            val call = BasicAuthClientDefault<PumpPlatformLotXidInterface>(
                mobileUser.userName!!,
                mobileUser.password!!,
                application.applicationContext
            )
                .create(PumpPlatformLotXidInterface::class.java)
                .getLotXid(params)

            call.enqueue(object : Callback<List<PumpPlatformSynchronize>> {
                override fun onFailure(
                    call: Call<List<PumpPlatformSynchronize>>,
                    t: Throwable
                ) {
                ...
                override fun onResponse(
                    call: Call<List<PumpPlatformSynchronize>>,
                    response: Response<List<PumpPlatformSynchronize>>
                ) {
                    if (response.isSuccessful) {
                        GlobalScope.launch(Dispatchers.IO) {
                            ...
                            if (!list.isNullOrEmpty()) {
                                pumpPlatformRepository.saveListObjectSynchronized(list)
                            }
                            ...
                            if (!GlobalUtil.method.recoverSharedPreferences(
                                    spConfiguration,
                                    PARAMETER_PUMP_PLATFORM_MAX_XID
                                ).isNullOrEmpty()
                            ...
                            if (xidSp > 0) {
                                ...
                                val xidDatabase = pumpPlatformRepository.maxXid()
                            ...
                            if (!isSynchronizationBackground && list.isNullOrEmpty()) {
                                activity?.runOnUiThread {
                                    GlobalUtil.method.shortTimeMessageAlert(
                                        application.applicationContext,
                                        application.applicationContext.getString(R.string.not_values_pump_platform)
                                    )
                                }
                                isCompletedSynchronization = true
                            }
    ...
    private fun nextLotXid() {
        ...
        val pumpPlanTestLotXidController =
            PumpPlanTestLotXidController(
                application,
                txtViewMessageProgress,
                btnStartSynchronization,
                progressBarSynchronization,
                activity,
                pumpPlanTestRepository.maxXid()
            )
        pumpPlanTestLotXidController.load()
    }
}