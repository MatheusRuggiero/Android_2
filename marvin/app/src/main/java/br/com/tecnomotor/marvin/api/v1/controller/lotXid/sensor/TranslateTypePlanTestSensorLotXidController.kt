...
import br.com.tecnomotor.marvin.api.v1.controller.lotXid.valve.ValveLotXidController
import br.com.tecnomotor.marvin.api.v1.interfaces.lotXid.sensor.TranslateTypePlanTestSensorLotXidInterface
...
class TranslateTypePlanTestSensorLotXidController(
        ...
        try {
            ...
            if (txtViewMessageProgress != null) {
                activity?.runOnUiThread {
                    txtViewMessageProgress!!.text = application.applicationContext.getString(R.string.translate_type_plan_test_sensor)
            ...
            val call = BasicAuthClientDefault<TranslateTypePlanTestSensorLotXidInterface>(
                mobileUser.userName!!,
                mobileUser.password!!,
                application.applicationContext
            )
                .create(TranslateTypePlanTestSensorLotXidInterface::class.java)
                .getLotXid(params)

            call.enqueue(object : Callback<List<TranslateTypePlanTestSensorSynchronize>> {
                override fun onFailure(
                    call: Call<List<TranslateTypePlanTestSensorSynchronize>>,
                    t: Throwable
                ) {
                ...
                override fun onResponse(
                    call: Call<List<TranslateTypePlanTestSensorSynchronize>>,
                    response: Response<List<TranslateTypePlanTestSensorSynchronize>>
                ) {
                    if (response.isSuccessful) {
                        GlobalScope.launch(Dispatchers.IO) {
                            ...
                            if (!list.isNullOrEmpty()) {
                                translateTypePlanTestSensorRepository.saveListObjectSynchronized(list)
                            }
                            ...
                            if (xidSp > 0) {
                                ...
                                val xidDatabase = translateTypePlanTestSensorRepository.maxXid()
                            ...
                            if (!isSynchronizationBackground && list.isNullOrEmpty()) {
                                activity?.runOnUiThread {
                                    GlobalUtil.method.shortTimeMessageAlert(
                                        application.applicationContext,
                                        application.applicationContext.getString(R.string.not_values_translate_type_plan_test_sensor)
                                    )
                                }
                                isCompletedSynchronization = true
                            }
    ...
    private fun nextLotXid() {
        ...
        val valveLotXidController =
            ValveLotXidController(
                application,
                txtViewMessageProgress,
                btnStartSynchronization,
                progressBarSynchronization,
                activity,
                valveRepository.maxXid()
            )
