...
import br.com.tecnomotor.marvin.api.v1.interfaces.maxXid.pump.TranslateTypePlanTestPumpMaxXidInterface
...
class TranslateTypePlanTestPumpMaxXidController(
            ...
            if (txtViewMessageProgress != null) {
                activity?.runOnUiThread {
                    txtViewMessageProgress!!.text =
                        application.applicationContext.getString(R.string.max_xid_translate_type_plan_test_pump)
                    btnStartSynchronization!!.isEnabled = false
                }
                isSynchronizationBackground = false
            }

            val call = BasicAuthClientDefault<TranslateTypePlanTestPumpMaxXidInterface>(
                mobileUser.userName!!,
                mobileUser.password!!,
                application.applicationContext
            )
                .create(TranslateTypePlanTestPumpMaxXidInterface::class.java)
                .getMax(params)
                        ...
                        GlobalScope.launch(Dispatchers.IO) {
                            ...
                            if (valuesMaxXid != null
                                ...
                                if (!isSynchronizationBackground) {
                                    GlobalUtil.method.shortTimeMessageAlert(
                                        application.applicationContext,
                                        application.applicationContext.getString(R.string.not_values_translate_type_plan_test_pump_max_xid)
                                    )
                                }
                            }
                            ...
                            if (xidSp == 0
                                && !GlobalUtil.method.recoverSharedPreferences(
                                    spConfiguration,
                                    PARAMETER_TRANSLATE_TYPE_PLAN_PUMP_MAX_XID_TRY_AGAIN
                                ).isNullOrEmpty()
                                && !GlobalUtil.method.recoverSharedPreferences(
                                    spConfiguration,
                                    PARAMETER_TRANSLATE_TYPE_PLAN_PUMP_MAX_XID_TRY_AGAIN
                                ...
                                if (xidSp == 0) {
                                    ...
                                    val translateTypePlanTestPumpMaxXidController =
                                        TranslateTypePlanTestPumpMaxXidController(
                                            application,
                                    ...
                                    translateTypePlanTestPumpMaxXidController.load()
    ...
    fun nextMaxXid() {
        ...
        val translateTypePointTestPumpMaxXidController = TranslateTypePointTestPumpMaxXidController(
            application,
            txtViewMessageProgress,
            btnStartSynchronization,
            progressBarSynchronization,
        ...
        translateTypePointTestPumpMaxXidController.load()
