...
import br.com.tecnomotor.marvin.api.v1.interfaces.lotXid.sensor.SensorPlatformLotXidInterface
...
import br.com.tecnomotor.marvin.repository.sensor.SensorPlatformRepository
...
class SensorPlatformLotXidController(
        ...
        try {
            ...
            if (txtViewMessageProgress != null) {
                activity?.runOnUiThread {
                    txtViewMessageProgress!!.text = application.applicationContext.getString(R.string.sensor_platform)
            ...
            val call = BasicAuthClientDefault<SensorPlatformLotXidInterface>(
                mobileUser.userName!!,
                mobileUser.password!!,
                application.applicationContext
            )
                .create(SensorPlatformLotXidInterface::class.java)
                .getLotXid(params)

            call.enqueue(object : Callback<List<SensorPlatformSynchronize>> {
                override fun onFailure(
                    call: Call<List<SensorPlatformSynchronize>>,
                    t: Throwable
                ) {
                ...
                override fun onResponse(
                    call: Call<List<SensorPlatformSynchronize>>,
                    response: Response<List<SensorPlatformSynchronize>>
                ) {
                    if (response.isSuccessful) {
                        GlobalScope.launch(Dispatchers.IO) {
                            val list = response.body()
                            val sensorPlatformRepository = SensorPlatformRepository(application)
                            var isCompletedSynchronization = false
                            if (!list.isNullOrEmpty()) {
                                sensorPlatformRepository.saveListObjectSynchronized(list)
                            }
                            ...
                            if (xidSp > 0) {
                                ...
                                val xidDatabase = sensorPlatformRepository.maxXid()

                                if (xidSp >= (xidDatabase + 1)) {
                                    //delay(20)
                                    val sensorPlatformLotXidController =
                                        SensorPlatformLotXidController(
                                            application,
                                    ...
                                    sensorPlatformLotXidController.load()
                                } else {
                            ...
                            if (!isSynchronizationBackground && list.isNullOrEmpty()) {
                                activity?.runOnUiThread {
                                    GlobalUtil.method.shortTimeMessageAlert(
                                        application.applicationContext,
                                        application.applicationContext.getString(R.string.not_values_sensor_platform)
                                    )
                                }
                                isCompletedSynchronization = true
                            }
    ...
    private fun nextLotXid() {
        val sensorPlanTestRepository = SensorPlanTestRepository(application)
        val sensorPlanTestLotXidController =
            SensorPlanTestLotXidController(
                ...
                progressBarSynchronization,
                activity,
                sensorPlanTestRepository.maxXid()
            )
