...
class ParameterRemoteMaxXidController(
        ...
        try {
            ...
            if (txtViewMessageProgress != null) {
                activity?.runOnUiThread {
                    txtViewMessageProgress!!.text = application.applicationContext.getString(R.string.max_xid_parameter_remote)
                        ...
                        GlobalScope.launch(Dispatchers.IO) {
                            ...
                            if (valuesMaxXid != null
                                ...
                                GlobalUtil.method.saveSharedPreferences(
                                    spConfiguration,
                                    PARAMETER_PARAMETER_REMOTE_MAX_XID,
                                    valuesMaxXid.xid
                                )
                                ...
                                if (!isSynchronizationBackground) {
                                    GlobalUtil.method.shortTimeMessageAlert(
                                        application.applicationContext,
                                        application.applicationContext.getString(R.string.not_values_parameter_remote_max_xid)
                                    )
                                }
                            }
                            ...
                            if (xidSp == 0
                                && !GlobalUtil.method.recoverSharedPreferences(
                                    spConfiguration,
                                    PARAMETER_PARAMETER_REMOTE_MAX_XID_TRY_AGAIN
                                ).isNullOrEmpty()
                                && !GlobalUtil.method.recoverSharedPreferences(
                                    spConfiguration,
                                    PARAMETER_PARAMETER_REMOTE_MAX_XID_TRY_AGAIN
                                ...
                                if (xidSp == 0) {
                                    ...
                                    val parameterRemoteMaxXidController =
                                        ParameterRemoteMaxXidController(
                                            application,
    ...
    fun nextMaxXid() {
        val permissionParameterRemoteMaxXidController = PermissionParameterRemoteMaxXidController(
            application,
            txtViewMessageProgress,
            btnStartSynchronization,
            progressBarSynchronization,
