...
import br.com.tecnomotor.marvin.api.v1.controller.maxXid.configuration.ParameterRemoteMaxXidController
import br.com.tecnomotor.marvin.api.v1.interfaces.maxXid.pump.PointTestPumpMaxXidInterface
...
class PointTestPumpMaxXidController(
        ...
        try {
            ...
            if (txtViewMessageProgress != null) {
                activity?.runOnUiThread {
                    txtViewMessageProgress!!.text = application.applicationContext.getString(R.string.max_xid_point_test_pump)
                    btnStartSynchronization!!.isEnabled = false
                }
                isSynchronizationBackground = false
            }

            val call = BasicAuthClientDefault<PointTestPumpMaxXidInterface>(
                mobileUser.userName!!,
                mobileUser.password!!,
                application.applicationContext
            )
                .create(PointTestPumpMaxXidInterface::class.java)
                .getMax(params)
                        ...
                        GlobalScope.launch(Dispatchers.IO) {
                            ...
                            if (valuesMaxXid != null
                                ...
                                if (!isSynchronizationBackground) {
                                    GlobalUtil.method.shortTimeMessageAlert(
                                        application.applicationContext,
                                        application.applicationContext.getString(R.string.not_values_point_test_pump_max_xid)
                                    )
                                }
                            }
                            ...
                            if (xidSp == 0
                                && !GlobalUtil.method.recoverSharedPreferences(
                                    spConfiguration,
                                    PARAMETER_POINT_TEST_PUMP_MAX_XID_TRY_AGAIN
                                ).isNullOrEmpty()
                                && !GlobalUtil.method.recoverSharedPreferences(
                                    spConfiguration,
                                    PARAMETER_POINT_TEST_PUMP_MAX_XID_TRY_AGAIN
                                ...
                                if (xidSp == 0) {
                                    ...
                                    val pointPumpTestMaxXidController =
                                        PointTestPumpMaxXidController(
                                            application,
                                    ...
                                    pointPumpTestMaxXidController.load()
    ...
    fun nextMaxXid() {
        ...
        val parameterRemoteMaxXidController = ParameterRemoteMaxXidController(
            application,
            txtViewMessageProgress,
            btnStartSynchronization,
            progressBarSynchronization,
        ...
        parameterRemoteMaxXidController.load()
